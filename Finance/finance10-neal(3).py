...
class stock_query():
    ...
    def __init__(self, next_stock, next_source, next_query_start, next_query_end=dt.datetime.now()):
        ...
        if isinstance(next_query_start, dt.date):
            self.query_start = next_query_start
        else:
            self.query_start = dt.datetime(*next_query_start)
...
# ------------------------------------------------------------------------------
# Need to add way to only add columns of interest. Right now adding the two columns that we already have the ability to calculate
# Want way for user to be able to put in equation and have it added to data frame
def get_and_plot_prices(stock_ticker_symbol, stock_data_source, stock_query_start, stock_query_end, x_axis_display, column_names ):
    
    stock = stock_query(stock_ticker_symbol, stock_data_source, stock_query_start, stock_query_end)
    
    stock_data = import_stock_data(stock.ticker_symbol, stock.data_source, stock.query_start, stock.query_end)
   
    ADD_percent_change(stock_import)
    ADD_Daily_Gain(stock_import)
    
    y = getDataFrameEnum(stock_ticker_symbol, stock_import.columns)
    stock_import.name = 'Yahoo NFLX: from %s to %s' % (stock_query_start, stock_query_end)
    
    plot_prices(stock_data, y.Date.display,[y.Daily_Gain.display, y.Percent_Change.display, y.Close.display] )
    
    
    return(stock_import)
    return(y)

# end get_and_plot_prices
# ------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    elif action == "get_and_plot_prices":
        NFLX = get_and_plot_prices(x.ticker_symbol,x.data_source,x.query_start,x.query_end,1,1)
